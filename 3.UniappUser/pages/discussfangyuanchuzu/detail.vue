...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfangyuanchuzuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfangyuanchuzuCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussfangyuanchuzu', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfangyuanchuzu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussfangyuanchuzu');
			...
			async init(){
				...
				let res = await this.$api.info('discussfangyuanchuzu', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfangyuanchuzu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussfangyuanchuzu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
