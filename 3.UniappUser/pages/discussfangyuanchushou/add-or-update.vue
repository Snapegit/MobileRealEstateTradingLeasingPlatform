	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussfangyuanchushou`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussfangyuanchushou`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussfangyuanchushou`, this.ruleForm);
						}else{
							await this.$api.add(`discussfangyuanchushou`, this.ruleForm);
						}
						uni.setStorageSync('discussfangyuanchushouCleanType',true);
