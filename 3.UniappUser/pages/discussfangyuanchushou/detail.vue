...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfangyuanchushouCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfangyuanchushouCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussfangyuanchushou', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfangyuanchushou');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussfangyuanchushou');
			...
			async init(){
				...
				let res = await this.$api.info('discussfangyuanchushou', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfangyuanchushou', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussfangyuanchushou/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
