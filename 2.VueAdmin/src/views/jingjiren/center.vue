				...
				<el-row>
					<el-col :span="24">
						<el-form-item label="经纪人账号" prop="jingjirenzhanghao">
							<el-input class="list_inp" v-model="user.jingjirenzhanghao" readonly placeholder="经纪人账号" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="经纪人姓名" prop="jingjirenxingming">
							<el-input class="list_inp" v-model="user.jingjirenxingming"  placeholder="经纪人姓名" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="头像" prop="touxiang">
							<uploads
								...
								@change="jingjirentouxiangUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="性别" prop="xingbie">
							<el-select 
								...
								placeholder="请选择性别"
								>
								<el-option v-for="item in jingjirenxingbieLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="门店" prop="mendian">
							<el-select 
								class="list_sel" 
								v-model="user.mendian" 
								placeholder="请选择门店"
								>
								<el-option v-for="item in jingjirenmendianLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const tableName = ref('jingjiren')
	const user = ref({})
	const jingjirenxingbieLists = ref([])
	const jingjirenmendianLists = ref([])
	const init = () => {
		jingjirenxingbieLists.value = "男,女".split(',')
		context?.$http({
			url:`option/mendian/mendian`,
			method:'get'
		}).then(res=>{
			jingjirenmendianLists.value = res.data.data
		})
	}
	const jingjirentouxiangUploadSuccess=(fileUrls)=> {
	    user.value.touxiang = fileUrls;
	}
	const onSubmit = () => {
		if((!user.value.jingjirenzhanghao)){
			context?.$toolUtil.message(`经纪人账号不能为空`,'error')
			return false
		}
		if((!user.value.mima)){
		...
		if((!user.value.jingjirenxingming)){
			context?.$toolUtil.message(`经纪人姓名不能为空`,'error')
			return false
		}
