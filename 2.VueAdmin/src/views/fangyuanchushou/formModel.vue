		...
		<el-dialog v-model="formVisible" :title="formTitle" width="50%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="销售价格" prop="xiaoshoujiage">
							<el-input class="list_inp" v-model.number="form.xiaoshoujiage" placeholder="销售价格"
								 type="number" 								:readonly="!isAdd||disabledForm.xiaoshoujiage?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="经纪人账号" prop="jingjirenzhanghao">
							<el-input class="list_inp" v-model="form.jingjirenzhanghao" placeholder="经纪人账号"
								 type="text" 								:readonly="!isAdd||disabledForm.jingjirenzhanghao?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="经纪人姓名" prop="jingjirenxingming">
							<el-input class="list_inp" v-model="form.jingjirenxingming" placeholder="经纪人姓名"
								 type="text" 								:readonly="!isAdd||disabledForm.jingjirenxingming?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="门店" prop="mendian">
							<el-input class="list_inp" v-model="form.mendian" placeholder="门店"
								 type="text" 								:readonly="!isAdd||disabledForm.mendian?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="定金" prop="dingjin">
							<el-input class="list_inp" v-model.number="form.dingjin" placeholder="定金"
								 type="number" 								:readonly="!isAdd||disabledForm.dingjin?true:false" />
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.fangwujieshao = res.data.data.fangwujieshao?(res.data.data.fangwujieshao.replace(reg,'../../../cl7459339/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='xiaoshoujiage'){
					form.value.xiaoshoujiage = row[x];
					disabledForm.value.xiaoshoujiage = true;
					continue;
				}
				if(x=='fangwujieshao'){
				...
				if(x=='jingjirenzhanghao'){
					form.value.jingjirenzhanghao = row[x];
					disabledForm.value.jingjirenzhanghao = true;
					continue;
				}
				if(x=='jingjirenxingming'){
					form.value.jingjirenxingming = row[x];
					disabledForm.value.jingjirenxingming = true;
					continue;
				}
				if(x=='mendian'){
					form.value.mendian = row[x];
					disabledForm.value.mendian = true;
					continue;
				}
				if(x=='storeupnum'){
				...
				if(x=='dingjin'){
					form.value.dingjin = row[x];
					disabledForm.value.dingjin = true;
					continue;
				}
				if(x=='zhuangtai'){
		...
		context?.$http({
			...
			if(json.hasOwnProperty('jingjirenzhanghao')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.jingjirenzhanghao = json.jingjirenzhanghao
				disabledForm.value.jingjirenzhanghao = true;
			}
			if(json.hasOwnProperty('jingjirenxingming')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.jingjirenxingming = json.jingjirenxingming
				disabledForm.value.jingjirenxingming = true;
			}
			if(json.hasOwnProperty('mendian')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.mendian = json.mendian
				disabledForm.value.mendian = true;
			}
		})
		context?.$http({
		...
		zhuangtaiLists.value = "已出售,未出售".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
